...
let main argv =
    ...
    if args.TryGetResult Version <> None then
        ...
        let loaderFunc : LoaderFunc =
            ...
            | _, Some _, Some _ -> fun (p, opts) -> BuildalyzerWorkspace.BuildalyzerWorkspaceLoader.Create(p, true, opts)
            | _, Some _, None -> fun (p, opts) -> BuildalyzerWorkspace.BuildalyzerWorkspaceLoader.Create(p, false, opts)
